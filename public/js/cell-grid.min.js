Cell.Grid=function(){function m(a){var b=m.prototype;this.options=a||{};this._domNode=this._xhr=null;this._sort={};this._auto_increment=0;this.options.pageDataCount=!1===this.options.pageDataCount?this.options.pageDataCount:15;this.options.selector=!!this.options.selector||!1;this._noMore=!1;q(this);r(this);t(this);u(this);v(this);b.getItemCount=function(){return $(this._domNode).find(".cell-grid-body-data tr").length};b.getItem=function(b){return $(this._domNode).find(".cell-grid-row[data-id="+b+
"]")[0]};b.getItems=function(b=!1){return $(this._domNode).find(`.cell-grid-body-data .cell-grid-row${b?':not(".disabled")':""}`).toArray()};b.getItemData=function(b,a=null){return a?$(b).data(a):$(b).data()};b.getItemsData=function(b=null,a=null){a?Array.isArray(a)||(a=[a]):a=$(this._domNode).find(".cell-grid-body-data .cell-grid-row");let c=[],d=this;$.each(a,function(a,f){(a=d.getItemData(f,b))&&(Array.isArray(a)?c=c.concat(Array.from(a)):c.push(a))});return c};b.getItemId=function(a){let b=[];
a=Array.isArray(a)?a:[a];$.each(a,function(a,c){b.push($(c).attr("data-id"))});return b};b.setSort=function(a){var b=$(this._domNode).find(".sortable[data-field-name='"+a+"']");this.reset();$(this._domNode).find(".cell-grid-header-table .sortable").not(b).removeClass("up").removeClass("down");$(b).hasClass("up")?($(b).removeClass("up").addClass("down"),b="DESC"):$(b).hasClass("down")?($(b).removeClass("down").addClass("up"),b="ASC"):($(b).addClass("down"),b="DESC");this._sort={field:a,type:b};this.load();
return this};b.setSelection=function(a,b=!0){a=$(a).not(".disabled");b&&a.removeClass("selected");a.toggleClass("selected").find(".cell-grid-row-checkbox").prop("checked",a.hasClass("selected"));n(this);this.options.onselected&&this.options.onselected.call(this,this.getSelection());return this};b.selectAll=function(){n(this)?this.removeSelection(this.getItems()):this.setSelection(this.getItems());return this};b.removeSelection=function(a,b=!0){$(this._domNode).find(".cell-grid-select-all").removeClass("selected").prop("checked",
...
l(e,g,c,c.name,a[c.name]))});$(this._domNode).find(".cell-grid-select-all").removeClass("selected").prop("checked",!1);w(this,g);h(e);return c};b.setItem=function(a,b){let c=this;Array.isArray(a)||(a=[a]);$.each(a,(a,d)=>{let e=c.getItem(d);if(!e)return console.error(`Not find item: ${d}`);p(c,e,b);$.each(b,function(a,b){let d=$c.getObjects(c.options.fields,"name",a);l(c,e,d,a,b)})});return this};b.setItemColVal=function(a,b,f){let c=this;Array.isArray(a)||(a=[a]);$.each(a,function(a,d){a=c.getItem(d);
d=$c.getObjects(c.options.fields,"name",b);$(a).data(b,f);l(c,a,d,b,f)});return this};b.renderItem=function(a,b=null){let c=this;Array.isArray(a)||(a=[a]);Array.isArray(b)||(b=[b]);let e=[];b?$(b).each(function(a,b){e.push($c.getObjects(c.options.fields,"name",b))}):e=this.options.fields;$.each(a,function(a,b){let d=c.getItem(b);$(e).each(function(a,b){a=$(d).find(`.${b.name}`);let e=$(d).data(b.name);$(a).html("").html(b.formatter?b.formatter.call(c,d,a,e):e)})});return this};b.reset=function(){this.options.onreset&&
this.options.onreset.call(this);this._noMore=!1;$(this._domNode).find(".cell-grid-body-data").empty();h(this);this.options.onselected&&this.options.onselected.call(this,this.getSelection())};b.load=function(){x(this)};b.setQuery=function(a){this.options.query=$.extend(this.options.query,a);this.reset();this.load();return this};b.reload=function(){$(".tooltip.bs-tooltip-top").remove();this.setQuery();return this};b.setUrl=function(a){this.options.url=a;this.reload();return this};b.getFirstItem=function(){return $(this._domNode).find(".cell-grid-body-data .cell-grid-row:first-child")};
b.checkDataCount=function(){h(this);return this};this.options.url?this.load():this.addItems(this.options.data)}var q=a=>{let b=void 0;a.options.replaceTo?b=$(a.options.replaceTo):(b=document.createElement("div"),$(b).appendTo(a.options.appendTo||".cell-main-container"));$(b).addClass("cell-grid").attr({id:a.options.id||null,tabindex:0}).on("keydown",function(b){b.ctrlKey&&65===b.keyCode&&(a.selectAll(),b.preventDefault())});a._domNode=b;!1!==a.options.header&&$(b).addClass("border")},t=a=>{if(!1!==
...
$.extend(b,a.options.query||{});return b},x=a=>{!a.options.url||a._noMore?h(a):(a._xhr&&a._xhr.abort(),$(a._domNode).find(".cell-grid-empty").hide(),$(a._domNode).find(".cell-grid-loading").show(),a._xhr=$c.ajax.get({url:a.options.url,data:y(a),success:function(b){a.addItems(b);!b||b&&b.length<a.options.pageDataCount?a._noMore=!0:k(a);$(a._domNode).find(".cell-grid-select-all").removeClass("selected").prop("checked",!1);a.options.onload&&a.options.onload.call(a)},error:function(b){a.options.onerror&&
a.options.onerror.call(a,b);h(a)}}))},h=a=>{let b=$(a._domNode).find(".cell-grid-empty"),c=$(a._domNode).find(".cell-grid-select-all");$(a._domNode).find(".cell-grid-loading").hide();0===a.getItemCount()?(b.show(),c.hide()):(b.hide(),c.show())},w=(a,b)=>{a.options.onaddItem&&a.options.onaddItem.call(a,b);$(b).hasClass("disabled")&&$(b).find(".cell-grid-row-selector").off("click").find(".cell-grid-row-checkbox").prop("disabled",!0);$(b).on("selectstart",!1).on("mouseover",function(c){a.options.onmouseover&&
a.options.onmouseover.call(a,c,b)})},p=(a,b,c)=>{$.each(a.options.fields,(a,f)=>{$(b).data(f.name,c[f.name])})},l=(a,b,c,d,f)=>{d=$(b).find("."+d.toString().replace(/_/g,"-"));$(d).html("").html(c.formatter?c.formatter.call(a,b,d,f):f)},k=a=>{if(!1!==a.options.pageDataCount&&a._xhr&&1!==a._xhr.readyState){var b=$(a._domNode);if(!0===b.is(":visible")){b=b.scrollParent();var [c,d,f]=[b.is(document)?$(window).height():b.innerHeight(),b.is(document)?b.height():b[0].scrollHeight,b.scrollTop()];c+f>=d-
2&&a.load()}}},r=a=>{if(!0===a.options.selector){var b=$(document.createElement("input")).attr("type","checkbox").addClass("cell-grid-select-all").hide();a.options.fields.unshift({name:"cell-grid-row-selector",label:b,formatter:function(b,d){$(document.createElement("input")).appendTo(d).attr("type","checkbox").addClass("cell-grid-row-checkbox");$(d).on("click",function(c){a.selectItem(b);c.stopPropagation()});$(d).on("dblclick",function(a){a.stopPropagation()})}})}},n=a=>{let b=a.getSelection().length===
a.getItems(!0).length;$(a._domNode).find(".cell-grid-select-all").prop("checked",b);return b};return m}();