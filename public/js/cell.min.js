...
!!a.account).html(a.ip);b&&$(document.createElement("span")).appendTo(d).attr("title",b).html($c.timeSince(b));return $(document.createElement("div")).append(c,d).children()};d.copy2cb=function(a){a.select();return document.execCommand("Copy")};d.banConsole=()=>{$("body").addClass("user-select-none");$(document).on("contextmenu",function(a){return a.preventDefault()});$(document).on("keydown",function(a){if(123===a.keyCode||a.ctrlKey&&a.shiftKey&&73===a.keyCode)return a.preventDefault()})};d.removeClassPrefix=
(a,b)=>{a.className=a.className.split(" ").filter(a=>!a.startsWith(b)).join(" ").trim();return this};d.createElement=(a,b,c,d)=>$(document.createElement(a)).appendTo(b).html(c).addClass(d);d.escape2Html=a=>$(document.createElement("div")).html(a).text();d.html2Escap=a=>$(document.createElement("div")).text(a).html();d.shuffleAarry=(a,b)=>{let c=-1,d=a.length;for(b=b||d;++c<b;){let b=c+Math.floor(Math.random()*(d-c)),e=a[b];a[b]=a[c];a[c]=e}a.length=b;return a};d.formatBytes=(a,b)=>{var c=["B","KB",
"MB","GB","TB"];let d=Math.floor((a?Math.log(a):0)/Math.log(1024));d=Math.min(d,c.length-1);c=c[d]?c[d]:c[0];return(a/(1<<10*d)).toFixed(b||2)+" "+c};d.setTitle=(a=!1,b=void 0)=>{!0===a&&$('.cell-mod-title[autoTitle="false"]').removeAttr("autoTitle");b=(b||$('.cell-mod-title[autoTitle!="false"]').text()).replace(/(^\s*)|(\s*$)|\s(?=\s)/g,"");document.title=(b&&`${b} - `)+document.title.split(" - ").pop();return this}}var $c=new Cell;